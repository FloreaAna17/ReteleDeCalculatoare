...
def record_operation(filename, user, action):
    import datetime
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if filename not in file_history:
        file_history[filename] = []
    file_history[filename].append(f"[{timestamp}] {user}: {action}")
        ...
        while True:
            ...
            try:
                ...
                # File operations
                    ...
                    record_operation(filename, current_user, "Created file")
                    ...
                    record_operation(filename, current_user, "Uploaded file")
                    ...
                    if not os.path.exists(old_filepath):
                        ...
                        record_operation(new_name, current_user, f"Renamed from '{old_name}'")
                    ...
                    if not os.path.exists(filepath):
                        ...
                        with open(filepath, 'r') as f:
                            content = f.read()
                        record_operation(filename, current_user, "Downloaded file")
                        response = {'status': 'success', 'content': content}
                    ...
                    if not os.path.exists(filepath):
                        ...
                        with open(filepath, 'w') as f:
                            f.write(content)
                        record_operation(filename, current_user, "Edited file")
                    ...
                    if not os.path.exists(filepath):
                        ...
                        response = {'status': 'success', 'history': history, 'message': f'Retrieved history for {filename}'}
