...
class FTPClient:
    ...
    def get_server_file_selection(self):
        """Helper to list files and ask user to select one"""
        command = {'command': 'list_files'}
        response = self.send_command(command)
        
        if response.get('status') != 'success':
            print(f"✗ {response.get('message', 'Error retrieving files')}")
            return None
    ...
    def read_file(self):
        ...
        filename = self.get_server_file_selection()
        if not filename:
            return
