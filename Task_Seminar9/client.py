...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat la server pentru a folosi aceasta comanda.")
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Comanda PUBLISH necesita un mesaj. Ex: PUBLISH Salut!")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if len(parti) < 2 or not parti[1].strip().isdigit():
            print("EROARE: Comanda DELETE necesita un ID numeric. Ex: DELETE 1")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
